...
app = Flask(__name__)
app.secret_key = 'your_secret_key'  # Asegúrate de cambiar esto en un entorno de producción
...
def perfil():
    if 'usuario' in session:
        ...
        return render_template('perfil.html', usuario=usuario)  # Renderizar la plantilla del perfil con el menú
...
def buscar_alumno_nombre():
    if request.method == 'POST':
        ...
        if alumno:
            solicitud = Solicitud.query.filter_by(id_alumno=alumno.idAlumno).first()
            if solicitud:
                return render_template('detalle_solicitud.html', alumno=alumno, solicitud=solicitud)
            else:
                return render_template('sin_solicitud.html', alumno=alumno)
        else:
            return render_template('alumno_no_encontrado.html')
